...
To simulate hockey games, we setup a Markov Chain where the states are a tuple of three consecutive
...
\end{align*}
Where $A,B,C,D$ are events that can occur in a game, and the tuple (A,B,C,D) represents that "A then 
B then C then D" happend right after eachother in a game. 
...
\While{$\text{time\_remaining} > 0$}
    ...
    \State $s0 \gets s1$
    \State $s1 \gets s2$
    \State $s2 \gets \text{next\_state}$
    
    \State $\text{event\_time} \gets \text{sample from } \text{KDE\_times()}$
    \State $\text{time\_remainting} \gets \text{time\_remainting} - \text{event\_time}$
\EndWhile
\end{algorithmic}
\end{algorithm}


% add the image images/markov_chain_sample.png
\begin{figure}[ht]
    \centering
